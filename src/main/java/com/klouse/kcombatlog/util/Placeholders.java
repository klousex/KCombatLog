package com.klouse.kcombatlog.util;
...
public final class Placeholders {
    ...
    public static String apply(Player player, String input, Map<String, String> replacements, boolean usePlaceholderApi) {
        ...
        for (Map.Entry<String, String> entry : replacements.entrySet()) {
            ...
            output = output.replace("%kcombatlog_" + entry.getKey() + "%", value);
        }

        if (player != null) {
            output = output.replace("%kcombatlog_player%", player.getName());
            output = output.replace("%kcombatlog_world%", player.getWorld().getName());
        }
