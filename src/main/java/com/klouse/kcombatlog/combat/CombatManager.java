...
import com.klouse.kcombatlog.api.event.KCombatLogTagEvent;
import com.klouse.kcombatlog.api.event.KCombatLogUntagEvent;
import com.klouse.kcombatlog.util.Placeholders;
...
public final class CombatManager {
    ...
    public CombatManager(KCombatLogPlugin plugin) {
        this.plugin = plugin;
        reload();
    }
    ...
    public void reload() {
        ...
        this.combatLogNotifyPermission = config.getString("settings.staff-notify.combat-log.permission", "kcombatlog.notify");
        this.combatLogNotifyFormat = config.getString("settings.staff-notify.combat-log.format", "&8[&cKCombatLog&8] &f{player} &7logged during combat. &8(&f{attacker}&8)");
    ...
    private void tagSingle(Player player, long expiresAt, UUID attacker, Player attackerPlayer, String cause) {
        ...
        Bukkit.getPluginManager().callEvent(new KCombatLogTagEvent(player, attackerPlayer, durationSeconds, cause));
        if (!wasTagged) {
    ...
    public void clear(UUID uuid, String reason) {
        ...
        if (player != null && player.isOnline()) {
            hideBossBar(player);
            if (removed != null) {
                Bukkit.getPluginManager().callEvent(new KCombatLogUntagEvent(player, reason));
            }
        }
    }
    ...
    public void notifyCombatLog(Player player, String reason) {
        ...
        String template = combatLogNotifyFormat == null || combatLogNotifyFormat.isBlank()
                ? "&8[&cKCombatLog&8] &f{player} &7logged during combat. &8(&f{attacker}&8)"
    ...
    private void tick() {
        ...
        for (Map.Entry<UUID, CombatTag> entry : new ArrayList<>(tags.entrySet())) {
            ...
            if (tag.expiresAtMillis() <= now) {
                ...
                Bukkit.getPluginManager().callEvent(new KCombatLogUntagEvent(player, "timer-expired"));
