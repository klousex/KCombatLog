...
import com.wobble.wcombatlog.api.event.WCombatLogTagEvent;
import com.wobble.wcombatlog.api.event.WCombatLogUntagEvent;
import com.wobble.wcombatlog.util.Placeholders;
...
public final class CombatManager {
    ...
    public CombatManager(WCombatLogPlugin plugin) {
        this.plugin = plugin;
        reload();
    }
    ...
    public void reload() {
        ...
        this.combatLogNotifyPermission = config.getString("settings.staff-notify.combat-log.permission", "wcombatlog.notify");
    ...
    private void tagSingle(Player player, long expiresAt, UUID attacker, Player attackerPlayer, String cause) {
        ...
        Bukkit.getPluginManager().callEvent(new WCombatLogTagEvent(player, attackerPlayer, durationSeconds, cause));
        if (!wasTagged) {
    ...
    public void clear(UUID uuid, String reason) {
        ...
        if (player != null && player.isOnline()) {
            hideBossBar(player);
            if (removed != null) {
                Bukkit.getPluginManager().callEvent(new WCombatLogUntagEvent(player, reason));
            }
        }
    }
    ...
    private void tick() {
        ...
        for (Map.Entry<UUID, CombatTag> entry : new ArrayList<>(tags.entrySet())) {
            ...
            if (tag.expiresAtMillis() <= now) {
                ...
                Bukkit.getPluginManager().callEvent(new WCombatLogUntagEvent(player, "timer-expired"));
