package com.wobble.wcombatlog.util;
...
public final class Placeholders {
    ...
    public static String apply(Player player, String input, Map<String, String> replacements, boolean usePlaceholderApi) {
        ...
        for (Map.Entry<String, String> entry : replacements.entrySet()) {
            ...
            output = output.replace("%wcombatlog_" + entry.getKey() + "%", value);
        }

        if (player != null) {
            output = output.replace("%wcombatlog_player%", player.getName());
            output = output.replace("%wcombatlog_world%", player.getWorld().getName());
        }
